...
actionControlsHtml = '' + teamLabelText + '';}playersList.innerHTML +=  <li> <div style="display: flex; align-items: center; gap: 10px;"> ${playerTeamLogoHtml} <div class="player-meta-box"> <div class="player-name-row"> <span class="player-name-clickable" onclick="openRoleModal(${index})">${player.name} ${roleSvgContent}</span> </div> <span class="player-mini-stats">${miniStatsString}</span> </div> </div> <div style="display: flex; align-items: center; gap: 5px;"> <button class="stats-btn" onclick="openPlayerModal(${index})" title="Просмотр статистики бойца">📊</button> ${actionControlsHtml} </div> </li>;});}/* 3. Рендер истории матчей */const historyList = document.getElementById('historyList');if (historyList) {if (matches.length === 0) {historyList.innerHTML = 'Игр пока не было';} else {historyList.innerHTML = matches.map(m => {let logo1Html = teams[m.t1] && teams[m.t1].img ? '' : '★';let logo2Html = teams[m.t2] && teams[m.t2].img ? '' : '★';let timeLabelHtml = m.time ? '' + m.time + '' : '';return '' +'' +'' + logo1Html + ' ' +'' + m.t1 + ' ' +'(' + m.s1 + ') : (' + m.s2 + ') ' +'' + m.t2 + ' ' +'' + logo2Html + '' +'' +timeLabelHtml +'';}).join('');}}}function updateMatchLogos() {const t1 = document.getElementById('matchTeam1').value;const t2 = document.getElementById('matchTeam2').value;const log1 = document.getElementById('logo-match-1');const log2 = document.getElementById('logo-match-2');if (log1 && t1 && teams[t1]) log1.innerHTML = teams[t1].img ? '' : '★';if (log2 && t2 && teams[t2]) log2.innerHTML = teams[t2].img ? '' : '★';}function openPlayerModal(index) {currentSelectedModalPlayerIndex = index;const player = players[index];document.getElementById('modalPlayerName').innerText = 'Боец: ' + player.name + ' (' + (player.role || 'Штурмовик') + ')';let kInp = document.getElementById('input-kills');let dInp = document.getElementById('input-deaths');let rInp = document.getElementById('input-revives');let bInp = document.getElementById('input-bombs');kInp.value = player.kills || 0;dInp.value = player.deaths || 0;rInp.value = player.revives || 0;bInp.value = player.bombs || 0;if (isAdmin) {kInp.removeAttribute('disabled'); dInp.removeAttribute('disabled');rInp.removeAttribute('disabled'); bInp.removeAttribute('disabled');document.getElementById('modalStatSaveBtn').style.display = 'block';document.getElementById('modalStatCancelBtn').innerText = 'Отмена';} else {kInp.setAttribute('disabled', 'true'); dInp.setAttribute('disabled', 'true');rInp.setAttribute('disabled', 'true'); bInp.setAttribute('disabled', 'true');document.getElementById('modalStatSaveBtn').style.display = 'none';document.getElementById('modalStatCancelBtn').innerText = 'Закрыть';}document.getElementById('playerStatsModal').style.display = 'flex';}async function savePlayerStats() {if (!isAdmin || currentSelectedModalPlayerIndex === null) return;let player = players[currentSelectedModalPlayerIndex];let k = parseInt(document.getElementById('input-kills').value);let d = parseInt(document.getElementById('input-deaths').value);let r = parseInt(document.getElementById('input-revives').value);let b = parseInt(document.getElementById('input-bombs').value);let kills = isNaN(k) || k < 0 ? 0 : k;let deaths = isNaN(d) || d < 0 ? 0 : d;let revives = isNaN(r) || r < 0 ? 0 : r;let bombs = isNaN(b) || b < 0 ? 0 : b;const { error } = await supabaseClient.from('players').update({ kills, deaths, revives, bombs }).eq('name', player.name);if (!error) {closePlayerModal();loadDataFromCloud();} else {alert('Ошибка сохранения в облако: ' + error.message);}}function closePlayerModal() {document.getElementById('playerStatsModal').style.display = 'none';currentSelectedModalPlayerIndex = null;}function openRoleModal(index) {if (!isAdmin) return;currentSelectedRolePlayerIndex = index;const player = players[index];document.getElementById('roleModalTitle').innerText = 'Роль для бойца: ' + player.name;document.getElementById('modalRoleSelect').value = player.role || "Штурмовик";document.getElementById('playerRoleModal').style.display = 'flex';}function closeRoleModal() {document.getElementById('playerRoleModal').style.display = 'none';currentSelectedRolePlayerIndex = null;}async function savePlayerRole() {if (!isAdmin || currentSelectedRolePlayerIndex === null) return;let player = players[currentSelectedRolePlayerIndex];let selectedRole = document.getElementById('modalRoleSelect').value;if (player.team !== "NoTeam" && selectedRole !== "Штурмовик") {let duplicateExists = players.some((p, idx) => {return idx !== currentSelectedRolePlayerIndex && p.team === player.team && (p.role || "Штурмовик") === selectedRole;});if (duplicateExists) {alert('Ошибка тактического лимита! В команде ' + player.team + ' уже есть один ' + selectedRole + '. Два специалиста одной роли недопустимы.');return;}}const { error } = await supabaseClient.from('players').update({ role: selectedRole }).eq('name', player.name);if (!error) {closeRoleModal();loadDataFromCloud();}}function openLogoModal(teamName) {if (!isAdmin) return;currentSelectedModalTeam = teamName;document.getElementById('modalTeamName').innerText = 'Логотип команды: ' + teamName;const teamData = teams[teamName];if (teamData.img) {document.getElementById('modalAddArea').style.display = 'none';document.getElementById('modalEditArea').style.display = 'block';document.getElementById('editLogoUrl').value = teamData.img;} else {document.getElementById('modalAddArea').style.display = 'block';document.getElementById('modalEditArea').style.display = 'none';document.getElementById('newLogoUrl').value = '';}document.getElementById('logoModal').style.display = 'flex';}function closeModal() {document.getElementById('logoModal').style.display = 'none';currentSelectedModalTeam = "";}async function saveModalLogo() {if (!isAdmin || !currentSelectedModalTeam) return;const teamData = teams[currentSelectedModalTeam];let url = teamData.img ? document.getElementById('editLogoUrl').value.trim() : document.getElementById('newLogoUrl').value.trim();if (url) {const { error } = await supabaseClient.from('teams').update({ img: url }).eq('name', currentSelectedModalTeam);if (!error) {closeModal();loadDataFromCloud();}} else {alert("Пожалуйста, введите ссылку!");}}async function deleteModalLogo() {if (!isAdmin || !currentSelectedModalTeam) return;if (confirm("Удалить логотип этой команды?")) {const { error } = await supabaseClient.from('teams').update({ img: "" }).eq('name', currentSelectedModalTeam);if (!error) {closeModal();loadDataFromCloud();}}}async function addTeam() {if (!isAdmin) return;const name = document.getElementById('teamName').value.trim();if (!name) return;let teamExists = Object.keys(teams).some(t => t.toLowerCase() === name.toLowerCase());if (teamExists) {alert('Ошибка! Команда с названием ' + name + ' уже зарегистрирована в турнирной системе.');return;}const { error } = await supabaseClient.from('teams').insert([{ name }]);if (!error) {document.getElementById('teamName').value = '';loadDataFromCloud();}}async function deleteTeam(teamName) {if (!isAdmin) return;if (confirm('Удалить команду ' + teamName + '?')) {await supabaseClient.from('players').update({ team: "NoTeam" }).eq('team', teamName);await supabaseClient.from('teams').delete().eq('name', teamName);loadDataFromCloud();}}async function addPlayer() {if (!isAdmin) return;const name = document.getElementById('playerName').value.trim();if (!name) return;let playerExists = players.some(p => p.name.toLowerCase() === name.toLowerCase());if (playerExists) {alert('Ошибка! Игрок с именем ' + name + ' уже зарегистрирован в турнирной системе.');return;}const { error } = await supabaseClient.from('players').insert([{ name, team: 'NoTeam', role: 'Штурмовик' }]);if (!error) {document.getElementById('playerName').value = '';loadDataFromCloud();}}async function deletePlayer(index) {if (!isAdmin) return;let player = players[index];if (confirm('Удалить игрока ' + player.name + '?')) {await supabaseClient.from('players').delete().eq('id', player.id);loadDataFromCloud();}}async function changePlayerTeam(playerIndex, newTeam) {if (!isAdmin) return;let player = players[playerIndex];let pRole = player.role || "Штурмовик";if (newTeam !== "NoTeam" && pRole !== "Штурмовик") {let duplicateExists = players.some((p, idx) => {return idx !== playerIndex && p.team === newTeam && (p.role || "Штурмовик") === pRole;});if (duplicateExists) {alert('Ошибка лимита! В команде ' + newTeam + ' уже зарегистрирован один ' + pRole + '. Сначала смените роль бойцу.');document.getElementById('select-p-' + playerIndex).value = player.team;return;}}const { error } = await supabaseClient.from('players').update({ team: newTeam }).eq('id', player.id);if (!error) {loadDataFromCloud();}}async function recordMatch() {if (!isAdmin) return;const t1 = document.getElementById('matchTeam1').value;const t2 = document.getElementById('matchTeam2').value;const s1 = parseInt(document.getElementById('score1').value);const s2 = parseInt(document.getElementById('score2').value);if (!t1 || !t2 || t1 === t2 || isNaN(s1) || isNaN(s2) || s1 < 0 || s2 < 0) {alert("Проверьте правильность заполнения команд и счета!");return;}let t1_pts = teams[t1].points, t1_w = teams[t1].wins, t1_d = teams[t1].draws, t1_l = teams[t1].losses;let t2_pts = teams[t2].points, t2_w = teams[t2].wins, t2_d = teams[t2].draws, t2_l = teams[t2].losses;if (s1 > s2) {t1_pts += 2; t1_w += 1; t2_l += 1;} else if (s2 > s1) {t2_pts += 2; t2_w += 1; t2_l += 1;} else {t1_pts += 1; t2_pts += 1; t1_d += 1; t2_d += 1;}let d = new Date();let timeString = String(d.getDate()).padStart(2, '0') + '.' + String(d.getMonth() + 1).padStart(2, '0') + '.' + d.getFullYear() + ' в ' + String(d.getHours()).padStart(2, '0') + ':' + String(d.getMinutes()).padStart(2, '0');await supabaseClient.from('matches').insert([{ t1: t1, s1: s1, t2: t2, s2: s2, time: timeString }]);await supabaseClient.from('teams').update({ points: t1_pts, wins: t1_w, draws: t1_d, losses: t1_l }).eq('name', t1);await supabaseClient.from('teams').update({ points: t2_pts, wins: t2_w, draws: t2_d, losses: t2_l }).eq('name', t2);document.getElementById('score1').value = '';document.getElementById('score2').value = '';loadDataFromCloud();}async function clearHistory() {if (!isAdmin) return;if (confirm("Очистить историю матчей и обнулить очки у всех команд?")) {await supabaseClient.from('matches').delete().neq('time', '');const sortedTeams = Object.keys(teams).filter(t => !teams[t].initial);for (let team of sortedTeams) {await supabaseClient.from('teams').update({ points: 0, wins: 0, draws: 0, losses: 0 }).eq('name', team);}loadDataFromCloud();}}/* Стартовая загрузка при открытии сайта */loadDataFromCloud();